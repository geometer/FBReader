...
GeometricCalculator::GeometricCalculator(const std::string &fileName) : ZLApplication("GeometricCalculator"), myBindings("Keys") {
	...
	toolbar().addButton(MODE_EDIT_OBJECT, "arrow", "Edit Object Properties", group);
	toolbar().addButton(MODE_ADD_POINT, "point", "New Point", group);
	toolbar().addButton(MODE_ADD_POINT_ON_THE_LINE, "pointontheline", "New Point On A Line", group);
	toolbar().addButton(MODE_ADD_MIDDLE_POINT, "middlepoint", "New Middle Point", group);
	toolbar().addSeparator();
	toolbar().addButton(MODE_ADD_LINE, "line", "New Line", group);
	toolbar().addButton(MODE_ADD_HALFLINE, "halfline", "New Halfline", group);
	toolbar().addButton(MODE_ADD_SEGMENT, "segment", "New Segment", group);
	toolbar().addSeparator();
	toolbar().addButton(MODE_ADD_PERPENDICULAR, "perpendicular", "New Perpendicular", group);
	toolbar().addButton(MODE_COPY_OBJECT, "copyobject", "Copy Object", group);
	toolbar().addButton(MODE_ADD_CIRCLE, "circle", "New Circle", group);
	toolbar().addSeparator();
	//toolbar().addButton(MODE_ADD_RULER, "ruler", "New Ruler", group);
	//toolbar().addSeparator();
	toolbar().addButton(MODE_MOVE_POINT, "movepoint", "Move Point", group);
	toolbar().addButton(MODE_DELETE, "delete", "Delete Object", group);
	toolbar().addSeparator();
	toolbar().addButton(ACTION_UNDO, "undo", "Undo");
	toolbar().addButton(ACTION_REDO, "redo", "Redo");
