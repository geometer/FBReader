...
GeometricCalculator::GeometricCalculator(const std::string &fileName) : ZLApplication("GeometricCalculator"), myBindings("Keys") {
	...
	toolbar().addButton(MODE_EDIT_OBJECT, ZLResourceKey("arrow"), group);
	toolbar().addButton(MODE_ADD_POINT, ZLResourceKey("point"), group);
	toolbar().addButton(MODE_ADD_POINT_ON_THE_LINE, ZLResourceKey("pointontheline"), group);
	toolbar().addButton(MODE_ADD_MIDDLE_POINT, ZLResourceKey("middlepoint"), group);
	toolbar().addSeparator();
	toolbar().addButton(MODE_ADD_LINE, ZLResourceKey("line"), group);
	toolbar().addButton(MODE_ADD_HALFLINE, ZLResourceKey("halfline"), group);
	toolbar().addButton(MODE_ADD_SEGMENT, ZLResourceKey("segment"), group);
	toolbar().addSeparator();
	toolbar().addButton(MODE_ADD_PERPENDICULAR, ZLResourceKey("perpendicular"), group);
	toolbar().addButton(MODE_COPY_OBJECT, ZLResourceKey("copyobject"), group);
	toolbar().addButton(MODE_ADD_CIRCLE, ZLResourceKey("circle"), group);
	toolbar().addSeparator();
	//toolbar().addButton(MODE_ADD_RULER, ZLResourceKey("ruler"), group);
	//toolbar().addSeparator();
	toolbar().addButton(MODE_MOVE_POINT, ZLResourceKey("movepoint"), group);
	toolbar().addButton(MODE_DELETE, ZLResourceKey("delete"), group);
	toolbar().addSeparator();
