...
void ZLDialogManager::wait(const ZLResourceKey &key, ZLRunnable &runnable, bool network) const {
	shared_ptr<ZLProgressDialog> dialog = createProgressDialog(key, network);
	if (!dialog.isNull()) {
		dialog->run(runnable);
	} else {
		runnable.run();
