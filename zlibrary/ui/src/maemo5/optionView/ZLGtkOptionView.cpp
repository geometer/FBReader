...
void BooleanOptionView::_createItem() {
	myCheckBox = hildon_check_button_new(HILDON_SIZE_FINGER_HEIGHT);
  gtk_button_set_label(GTK_BUTTON(myCheckBox), gtkString(name()).c_str());
	hildon_check_button_set_active(HILDON_CHECK_BUTTON(myCheckBox), ((ZLBooleanOptionEntry&)*myOption).initialState());
...
void ComboOptionView::_createItem() {
  ...
  myPickerButton = hildon_picker_button_new(HILDON_SIZE_FINGER_HEIGHT, HILDON_BUTTON_ARRANGEMENT_HORIZONTAL);
  hildon_button_set_title(HILDON_BUTTON(myPickerButton), gtkString(name()).c_str());

  hildon_picker_button_set_selector (HILDON_PICKER_BUTTON (myPickerButton), HILDON_TOUCH_SELECTOR (selector));

	g_signal_connect(GTK_WIDGET(myPickerButton), "value-changed", G_CALLBACK(_onValueChanged), this);

	myHolder.attachWidget(*this, GTK_WIDGET(myPickerButton));

	reset();
}

void ComboOptionView::_show() {
	gtk_widget_show(GTK_WIDGET(myPickerButton));
}

void ComboOptionView::_hide() {
	gtk_widget_hide(GTK_WIDGET(myPickerButton));
}

void ComboOptionView::_setActive(bool active) {
	gtk_widget_set_sensitive(GTK_WIDGET(myPickerButton), active);
}

void ComboOptionView::_onAccept() const {
  HildonTouchSelector *selector = hildon_picker_button_get_selector(HILDON_PICKER_BUTTON(myPickerButton));
	((ZLComboOptionEntry&)*myOption).onAccept(hildon_touch_selector_get_current_text(selector));
}
...
void ComboOptionView::reset() {
  ...
  if(myListSize > 0) {
	...
	for (std::vector<std::string>::const_iterator it = values.begin(); it != values.end(); ++it, ++index) {
		...
		hildon_touch_selector_append_text(selector, it->c_str());
	}

	if (mySelectedIndex >= 0) {
		hildon_touch_selector_set_active(selector, 0, mySelectedIndex);
	}
}

void ComboOptionView::onValueChanged() {
  HildonTouchSelector *selector = hildon_picker_button_get_selector(HILDON_PICKER_BUTTON(myPickerButton));
	int index = hildon_touch_selector_get_active(selector, 0);
...
void StringOptionView::_createItem() {
	myLineEdit = HILDON_ENTRY(hildon_entry_new(HILDON_SIZE_AUTO));
	g_signal_connect(GTK_WIDGET(myLineEdit), "changed", G_CALLBACK(_onValueChanged), this);
...
void StringOptionView::reset() {
	...
	hildon_entry_set_text(myLineEdit, ((ZLStringOptionEntry&)*myOption).initialValue().c_str());
}

void StringOptionView::onValueChanged() {
	ZLStringOptionEntry &o = (ZLStringOptionEntry&)*myOption;
	if (o.useOnValueEdited()) {
		o.onValueEdited(hildon_entry_get_text(myLineEdit));
	}
}
...
void StringOptionView::_onAccept() const {
	((ZLStringOptionEntry&)*myOption).onAccept(hildon_entry_get_text(myLineEdit));
}
