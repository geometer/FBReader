...
#include "ZLQtOptionViewHolder.h"
#include "ZLQtOptionView.h"

ZLOptionView *ZLQtOptionViewHolder::createViewByEntry(const std::string &name, const std::string &tooltip, shared_ptr<ZLOptionEntry> option) {
	if (option == 0) {
		return 0;
	}
	...
	switch (option->kind()) {
		...
		case ZLOptionEntry::MULTILINE:
			return new MultilineOptionView(name, tooltip, option, *this);
