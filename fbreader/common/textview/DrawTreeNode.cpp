	...
	switch (kind) {
		...
		case TreeElement::TREE_ELEMENT_OPEN_NODE:
		{
			...
			if ((kind == TreeElement::TREE_ELEMENT_CLOSED_NODE) ||
				...
				context().drawLine(x + size / 2, y + myStyle.style()->spaceAfter(), x + size / 2, y);
			}
			break;
		}
		...
		case TreeElement::TREE_ELEMENT_TOP_RIGHT_LINE:
		{
			int size = context().stringHeight();
			context().setColor(color);
			context().drawLine(x + size / 3 * 2, y - size / 3, x + size / 3 * 2, y - height + 1 - myStyle.style()->spaceBefore());
		...
		case TreeElement::TREE_ELEMENT_TOP_BOTTOM_RIGHT_LINE:
		{
			int size = context().stringHeight();
			context().setColor(color);
			context().drawLine(x + size / 3 * 2, y + myStyle.style()->spaceAfter(), x + size / 3 * 2, y - height + 1 - myStyle.style()->spaceBefore());
			context().drawLine(x + size / 3 * 2, y - size / 3, x + size * 4 / 3, y - size / 3);
			break;
		}
