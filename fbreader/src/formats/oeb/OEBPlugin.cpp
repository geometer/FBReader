...
bool OEBPlugin::acceptsFile(const ZLFile &file) const {
	...
	if (!mimeType.empty()) {
		return
			mimeType == ZLMimeType::APPLICATION_EPUB_ZIP->getName() ||
			(mimeType == ZLMimeType::APPLICATION_XML->getName() && extension == OPF) ||
			(mimeType == ZLMimeType::APPLICATION_ZIP->getName() && extension == OEBZIP);
	}
