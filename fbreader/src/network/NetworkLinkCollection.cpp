...
private:
	void run() {
		NetworkLinkCollection::LinkVector links;
		...
		for (NetworkLinkCollection::LinkVector::iterator it = links.begin(); it != links.end(); ++it) {
			shared_ptr<NetworkLink> link = *it;
			myNetworkLinkCollection.addOrUpdateLink(*link);
		}
